...
// HUOM: pdf-parse ladataan dynaamisesti vain kun sitä tarvitaan (Vercel-yhteensopivuus)

/**
 * Lataa kaikki brändiohjedokumentit tietokannasta
 */
export async function loadBrandGuidelines() {
  console.log('[loadBrandGuidelines] Aloitetaan haku...')

  if (!supabaseAdmin) {
    console.error('[loadBrandGuidelines] ❌ Supabase admin client missing')
    return []
  }

  console.log('[loadBrandGuidelines] ✅ Supabase admin client OK')

  try {
    // HUOM: Poistettu väliaikaisesti .eq('is_active', true) koska RLS-policy estää sen
    // Filtteröidään is_active manuaalisesti datan jälkeen
    console.log('[loadBrandGuidelines] Haetaan KAIKKI rivit (ilman is_active filtteriä)...')
    ...
    if (error) {
      console.error('[loadBrandGuidelines] ❌ Error loading guidelines:', error)
      return []
    }

    console.log(`[loadBrandGuidelines] ✅ Haettu ${data?.length || 0} riviä tietokannasta`)

    // Filtteröi is_active = true manuaalisesti
    const activeGuidelines = data?.filter(doc => doc.is_active === true) || []

    console.log(`[loadBrandGuidelines] ✅ Filtteröinnin jälkeen: ${activeGuidelines.length} aktiivista dokumenttia`)
    activeGuidelines.forEach((doc, i) => {
      console.log(`[loadBrandGuidelines]   ${i + 1}. ${doc.title} (status: ${doc.status}, is_active: ${doc.is_active})`)
    })

    return activeGuidelines
  } catch (error) {
    console.error('[loadBrandGuidelines] ❌ Exception loading guidelines:', error)
    return []
  }
}
...
export async function createBrandGuideline({ title, fileName, fileUrl, filePath, userId, userEmail }) {
  console.log('[createBrandGuideline] ======================================')
  console.log('[createBrandGuideline] Aloitetaan dokumentin luonti')

  if (!supabaseAdmin) {
    console.error('[createBrandGuideline] ❌ Supabase admin client puuttuu!')
    throw new Error('Supabase admin client missing')
  }

  console.log('[createBrandGuideline] ✅ Supabase admin client OK')
  console.log('[createBrandGuideline] Parametrit:', {
    title,
    fileName,
  ...
  try {
    ...
    console.log('[createBrandGuideline] Insert data:', JSON.stringify(insertData, null, 2))
    console.log('[createBrandGuideline] Suoritetaan INSERT...')
    ...
    if (error) {
      console.error('[createBrandGuideline] ❌ Error creating guideline:', {
        code: error.code,
    ...
    console.log('[createBrandGuideline] ✅ Dokumentti luotu onnistuneesti:', {
      id: data?.id,
      title: data?.title,
      status: data?.status
    })
    ...
    console.error('[createBrandGuideline] ❌❌❌ Exception creating guideline:', {
      name: error.name,
      message: error.message,
      code: error.code,
      details: error.details,
...
export async function readPDFContent(fileBuffer) {
  try {
    // Ladataan pdf-parse dynaamisesti vain kun sitä tarvitaan
    // Tämä välttää "DOMMatrix is not defined" virheen Vercelissä
