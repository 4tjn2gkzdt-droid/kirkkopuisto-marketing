...
export const filterPosts = (posts, filters = {}) => {
  ...
  if (!showPastEvents) {
    ...
    currentPosts = currentPosts.filter(post => {
      // Monipäiväiset tapahtumat: tarkista viimeinen päivä
      if (post.dates && Array.isArray(post.dates) && post.dates.length > 0) {
        const lastDate = post.dates[post.dates.length - 1];
        const eventDate = parseLocalDate(lastDate.date);
        eventDate.setHours(0, 0, 0, 0);
        return eventDate >= today;
      }

      // Backward compatibility: käytä post.date
...
export const getEventsForDate = (date, allPosts = []) => {
  ...
  // Tarkista sekä post.date (backward compatibility) että dates-array (monipäiväiset tapahtumat)
  return allPosts.filter(post => {
    // Tarkista ensimmäinen päivä (backward compatibility)
    if (post.date === dateStr) return true;

    // Tarkista monipäiväiset tapahtumat
    if (post.dates && Array.isArray(post.dates)) {
      return post.dates.some(dateEntry => dateEntry.date === dateStr);
    }
